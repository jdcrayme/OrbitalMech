...
public class GUIManager : MonoBehaviour {
    ...
    public void OnGUI()
    {
        ...
        if (SystemData != null)
        {
            GUI.Label(new Rect(10, 3, 100, 20), SystemData.name, Large);
            GUI.Label(new Rect(10, 30, 100, 20), SystemData.Designator, Small);
        }
    ...
    {
        const float hIndent = 10;
        const float vIndent = 10;

        DrawLine(new Vector2(hIndent, Screen.height - vIndent), new Vector2(Screen.width - hIndent, Screen.height - vIndent));

        var screenSize = OrthoCam.Instance.Size;
        var step = screenSize/10;


        var stepSize = (Screen.width - 2*hIndent)/10;

        for (int i = 0; i < 10; i++)
        {
            var x = hIndent + i*stepSize;

            DrawLine(new Vector2(x, Screen.height - vIndent), new Vector2(x, Screen.height - 3*vIndent));
            GUI.Label(new Rect(x+3, Screen.height - 3 * vIndent, 100, 20), ""+step*i, Small);

            DrawLine(new Vector2(x + stepSize/2, Screen.height - vIndent), new Vector2(x + stepSize/2, Screen.height - 2*vIndent));
        }
